<div id="modalRegistro"
    class="fixed inset-0 bg-gray-900 bg-opacity-60 hidden z-50 flex items-center justify-center backdrop-blur-sm transition-opacity">
    <div
        class="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto transform transition-all scale-100 font-sans">
        ...
        <div class="bg-[#B11A1A] px-6 py-4 flex justify-between items-center sticky top-0 z-20 shadow-md">
            ...
            <button type="button" onclick="closeModal()"
                class="text-white/80 hover:text-white text-2xl transition hover:scale-110">&times;</button>
        </div>
        ...
        <form action="{{ route('afiliados.store') }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-8">
            ...
            <div
                class="bg-gray-50 border border-gray-200 rounded-lg p-4 flex flex-col sm:flex-row justify-between items-center text-sm shadow-sm">
                <div class="flex items-center gap-2 mb-2 sm:mb-0">
                    <div class="w-8 h-8 rounded-full bg-[#fdf2f2] flex items-center justify-center text-[#B11A1A]">
                        <i class="fa-regular fa-calendar"></i>
                    </div>
                    <div>
                        <span class="block text-[#8B8889] font-bold text-xs uppercase tracking-wider">Fecha de
                            Registro</span>
                ...
                <div class="flex items-center gap-3">
                    <div
                        class="w-10 h-10 rounded-full bg-[#fdf2f2] flex items-center justify-center text-[#B11A1A] shrink-0">
                        <i class="fa-solid fa-user-pen"></i>
                    </div>
                    <div class="flex-1">
                        <label for="Orientador"
                            class="block text-[#8B8889] font-bold text-xs uppercase tracking-wider mb-1">
                            Orientador Responsable *
                        </label>
                        <select name="Orientador" id="Orientador" required
                            class="w-full bg-transparent border-none p-0 font-semibold text-gray-800 text-sm focus:ring-0 cursor-pointer hover:text-[#B11A1A] transition-colors">
                            <option value="" disabled selected>Seleccione orientador...</option>
                            @foreach ($vendedores as $vendedor)
                                <option value="{{ $vendedor->DES_VENDEDOR }}"
                                    {{ old('Orientador') == $vendedor->DES_VENDEDOR ? 'selected' : '' }}>
            ...
            <div>
                <h3 class="text-[#B11A1A] font-bold border-b border-gray-200 pb-2 mb-6 flex items-center gap-3 text-lg">
                    <span
                        class="bg-[#fdf2f2] text-[#B11A1A] border border-red-100 w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold shadow-sm">1</span>
                    Datos del Paciente
                </h3>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="group">
                        <label
                            class="block text-sm font-bold text-[#8B8889] mb-1.5 group-hover:text-[#B11A1A] transition-colors">DNI
                            *</label>
                        <input type="text" name="Afiliado_DNI" maxlength="8" value="{{ old('Afiliado_DNI') }}"
                            required placeholder="Ingrese 8 dígitos"
                    ...
                    <div class="group">
                        <label
                            class="block text-sm font-bold text-[#8B8889] mb-1.5 group-hover:text-[#B11A1A] transition-colors">Nombres
                            Completos *</label>
                    ...
                    <div class="group">
                        <label
                            class="block text-sm font-bold text-[#8B8889] mb-1.5 group-hover:text-[#B11A1A] transition-colors">Teléfono</label>
                    ...
                    <div class="group">
                        <label
                            class="block text-sm font-bold text-[#8B8889] mb-1.5 group-hover:text-[#B11A1A] transition-colors">Email</label>
                    ...
                    <div class="md:col-span-2 group">
                        <label
                            class="block text-sm font-bold text-[#8B8889] mb-1.5 group-hover:text-[#B11A1A] transition-colors">Dirección</label>
            ...
            <div class="bg-gray-50 p-6 rounded-xl border border-gray-200">
                <h3 class="text-[#8B8889] font-bold border-b border-gray-200 pb-2 mb-4 flex items-center gap-3 text-lg">
                    <span
                        class="bg-white text-[#8B8889] border border-gray-300 w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold shadow-sm">2</span>
                    Datos del Apoderado <span
                        class="text-xs font-normal text-gray-400 ml-auto bg-white px-2 py-1 rounded border border-gray-200">Opcional</span>
                </h3>
            ...
            <div>
                <h3
                    class="text-[#B11A1A] font-bold border-b border-gray-200 pb-2 mb-4 flex items-center gap-3 text-lg">
                    <span
                        class="bg-[#fdf2f2] text-[#B11A1A] border border-red-100 w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold shadow-sm">3</span>
                    Documentación
                </h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-end">
                    <div class="group">
                        <label
                            class="block text-sm font-bold text-[#8B8889] mb-2 group-hover:text-[#B11A1A] transition-colors">Contrato
                            Adjunto</label>
                        <input type="file" name="Contrato_adjunto"
                    ...
                    <div class="flex items-center pb-2">
                        <label
                            class="flex items-center space-x-3 cursor-pointer p-3 rounded-md border border-transparent hover:bg-gray-50 hover:border-gray-200 w-full transition-all">
                            <input type="checkbox" name="Tiene_Firma_Huella" value="1"
                                {{ old('Tiene_Firma_Huella') ? 'checked' : '' }}
            ...
            <div class="flex justify-end gap-4 pt-6 border-t border-gray-200">
                <button type="button" onclick="closeModal()"
                    class="px-6 py-2.5 rounded-lg border border-gray-300 text-gray-700 font-medium hover:bg-gray-100 transition shadow-sm">
                    Cancelar
                </button>
                <button type="submit"
                    class="px-8 py-2.5 rounded-lg bg-[#B11A1A] text-white font-bold shadow-md hover:bg-[#8f1515] hover:shadow-lg transition transform hover:-translate-y-0.5 flex items-center gap-2">
